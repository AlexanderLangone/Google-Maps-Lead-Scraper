document.addEventListener('DOMContentLoaded', function() {
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        ...
        var uniqueDomains = new Set(); // To store unique base domains
        ...
        actionButton.addEventListener('click', function() {
            chrome.scripting.executeScript({
                ...
                // Add new results to the table, ensuring no duplicate domains, skipping .edu websites, and filtering entries without websites or phone numbers
                if (!results || !results[0] || !results[0].result) return;
                results[0].result.forEach(function(item) {
                    if (!item.companyUrl || item.companyUrl.endsWith('.edu') || !item.phone) {
                        return; // Skip if no website URL, if the URL is a .edu domain, or if there's no phone number
                    }

                    let domain = extractDomain(item.companyUrl); // Extract base domain
                    if (uniqueDomains.has(domain)) {
                        return; // Skip if the base domain is a duplicate
                    }

                    uniqueDomains.add(domain); // Add base domain to the Set

                    // Update address with city and state info in the format "1495 NW 20th St, Miami, Florida"
                    const formattedAddress = extractFormattedAddress(item.address);
...
function extractDomain(url) {
    ...
    domain = domain.split(':')[0]; // Remove port number, if any
    domain = domain.replace('www.', ''); // Remove 'www.' if it exists
    return domain;
}

// Function to extract and format the address including city and state
function extractFormattedAddress(address) {
    // Assuming Google provides the address with a clear format like "1495 NW 20th St, Miami, Florida"
    const cityStateRegex = /, ([\w\s]+), ([\w\s]+)/;
    const match = address.match(cityStateRegex);
    if (match && match[1] && match[2]) {
        return `${address}, ${match[1]}, ${match[2]}`;
    }
    return address; // Return the original address if city and state are not found
}
